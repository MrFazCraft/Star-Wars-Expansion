...
public class DL44RightClickProcedureProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack itemstack) {
			...
			return;
		{
			...
			if (!projectileLevel.isClientSide()) {
				...
				}.getArrow(projectileLevel, 2, 0); // Daño conservado

				// Offset para mover el disparo hacia adelante y a la derecha
				Vec3 look = _shootFrom.getLookAngle();
				Vec3 right = look.cross(new Vec3(0, 1, 0)).normalize();

				double forwardOffset = 1.0; // Adelante
		...
		if (world instanceof Level _level) {
			if (!_level.isClientSide()) {
				_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("star_wars_expansion:blaster_shot")), SoundSource.NEUTRAL, 1, 1);
			} else {
				_level.playLocalSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("star_wars_expansion:blaster_shot")), SoundSource.NEUTRAL, 1, 1, false);
			}
		}
		if (itemstack.getItem() instanceof DL44Item)
			itemstack.getOrCreateTag().putString("geckoAnim", "1");
		if (entity instanceof Player _player)
			_player.getCooldowns().addCooldown(itemstack.getItem(), 14); // Cooldown conservado
	}
}
