...
public class EnemyLaserProjectileRenderer extends EntityRenderer<EnemyLaserProjectileEntity> {
    ...
    public EnemyLaserProjectileRenderer(EntityRendererProvider.Context context) {
        super(context);
        model = new Modellaser(context.bakeLayer(Modellaser.LAYER_LOCATION));
    }

    @Override
    public void render(EnemyLaserProjectileEntity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
        // Render normal
        ...
        // Render emisivo (glow) sin afectar la luz del mundo
        poseStack.pushPose();
        poseStack.mulPose(Axis.YP.rotationDegrees(Mth.lerp(partialTicks, entityIn.yRotO, entityIn.getYRot()) - 90));
        poseStack.mulPose(Axis.ZP.rotationDegrees(90 + Mth.lerp(partialTicks, entityIn.xRotO, entityIn.getXRot())));
    ...
    @Override
    public ResourceLocation getTextureLocation(EnemyLaserProjectileEntity entity) {
        return texture;
    }
}
